...
import org.apache.gluten.execution.{FilterExecTransformerBase, SupportPushDownPostScanFilters}
...
object PushDownPostScanFiltersToScan extends Rule[SparkPlan] with PredicateHelper {
  ...
  override def apply(plan: SparkPlan): SparkPlan = plan.transformUp {
    case filter: FilterExecTransformerBase =>
      filter.child match {
        case scan: SupportPushDownPostScanFilters =>
          val postScanFilters =
          ...
          if (postScanFilters.nonEmpty) {
            val newScan = scan.pushDownPostScanFilters(postScanFilters)
